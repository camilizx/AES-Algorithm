...
def galois_field_multiplication(a, b):
    ...
    if (b==1):
        ...
        print (a)
        print(b)
    #     temp = sub_word(a, l_table) + sub_word(b, l_table)
    #     if (temp > 0xFF):
    #         temp -= 0xFF
    # return sub_word(temp, e_table)

def mix_column(state):
    mixed_state = [[0 for _ in range(4)] for _ in range(4)]
    print (state)
    print(mix_column_table)
    print (mixed_state)
    for col in range(4):
        for row in range(4):
            mixed_state[row][col] = (
...
def main():
    ...
    print (galois_field_multiplication(0xAF, 0x08))
